...
from django_aauth.authentication import AAuthAgent, AAuthAuthentication
...
def test_authenticates_validly_signed_request():
    ...
    user, results = backend.authenticate(request)

    assert isinstance(user, AAuthAgent)
    assert user.agent_id == identity.agent_id
    assert len(results) == 1
    assert results[0].parameters["keyid"] == identity.agent_id


def test_authenticated_user_satisfies_is_authenticated_permission():
    """Regression test: DRF's built-in IsAuthenticated permission calls
    request.user.is_authenticated, which used to crash because
    authenticate() returned a bare str as the user."""
    identity = generate_identity("aauth:demo-agent@example.com")
    url = "http://testserver/resource"
    sig_headers = _sign("GET", url, identity.agent_id, identity.private_key)
    ...
    backend = _KnownAgentAuthentication({identity.agent_id: identity.public_key})
    user, _results = backend.authenticate(request)
    request.user = user
